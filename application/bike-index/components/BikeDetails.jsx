...
var BikeDetails = React.createClass({
  ...
  render: function() {
    var model = this.state.currentBike;
    if (!model) {
      return <div className="BikeDetails">Loading…</div>
    }
    return (
      <div className = "BikeDetails">
        <ul className="Photos">{
          model.publicImages.map( imageMetadata =>  
            <li 
              key={ imageMetadata.large }>
              <img src={ imageMetadata.large }/>
            </li>
          )
        }</ul>
        <div className="Body">
          ...
          <table className="Components"> 
            <tbody>{
              model.components.filter(component => 
                component.manufacturerName && component.modelName).map((component, i) => 
                  <tr key={i}>
